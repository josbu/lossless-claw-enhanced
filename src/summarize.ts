  ...
  const policy =
    mode === "aggressive"
      ? [
          ...
          "- ALWAYS preserve specific concrete values even in aggressive mode: numbers, numeric ranges, percentage thresholds, credentials, addresses, color codes, and any user-provided configuration values.",
          "- Preserve exact technical names (function names, event names, enum values) verbatim — never substitute with plausible alternatives.",
        ].join("\n")
      : [
          "Normal summary policy:",
          "- Preserve key decisions, rationale, constraints, and active tasks.",
          "- Keep essential technical details needed to continue work safely.",
          "- ALWAYS preserve specific concrete values: numbers, port numbers, IP addresses, phone numbers, email addresses, URLs, file paths, API keys/secrets, passwords, color codes, date/time values, version numbers, price/cost values, numeric ranges (e.g. 1-10, 11-100), percentage thresholds (e.g. 90%), and any other explicit configuration values mentioned by the user.",
          "- Preserve names of people, places, and organizations with their associated details.",
          "- When the conversation defines named lists (function names, event names, API endpoints, field names, enum values), preserve the EXACT names verbatim. NEVER paraphrase or invent alternative names — if you cannot fit all items, keep as many as possible with exact spelling rather than replacing them with plausible-sounding alternatives.",
          "- When the conversation defines rule tables (state transitions, permission matrices, priority orderings), preserve the COMPLETE set of rules, not just the general principle. For example, preserve all allowed/disallowed transitions, not just 'X is the terminal state'.",
          "- When the conversation compares or ranks options (A is better than B because...), preserve both the ranking ORDER and the REASONS, not just the winning choice.",
...
function buildD1Prompt(params: {
  ...
  return [
    ...
    [
      ...
      "- When options were compared or ranked (A chosen over B), preserve the ranking order AND the reasons, not just the winner.",
      ...
      "- Exact technical names (function names, event names, field names, enum values) — copy verbatim from the source, never paraphrase.",
      "- Complete rule sets (state machines, permission tables, transition matrices) — preserve all entries, not just the general rule.",
      "- ALL concrete values the user explicitly stated: port numbers, phone numbers, email addresses, API keys, secrets, passwords, color codes (#hex), prices, numeric ranges, percentage thresholds, dates/times, and configuration parameters. These are irreplaceable and must survive compaction verbatim.",
      "",
